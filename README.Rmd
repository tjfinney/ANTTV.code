...
Analysis results by specifying a file name and setting the *write* flag to `TRUE`:

```
Mark.UBS4 |> do_reduction(keep="Origen") |> do_dist() |> do_PAM(fn="../PAM/Mark.UBS4.txt", write=TRUE)
```

(*ASW* is the *average silhouette width*, also known as the *mean silhouette width*, *MSW*.)
