...
Details of the different analysis types (CMDS = classical multidimensional scaling, DC = divisive clustering, NJ = neighbour-joining, PAM = partitioning around medoids) can be found in the references. In PAM analysis, *ASW* is the *average silhouette width*, also known as the *mean silhouette width*, *MSW*. Peaks in *ASW* versus number of groups indicate better groupings.

Bootstrap sampling of an input data matrix can be used to produce a neighbour-joining tree with support values or the majority-rule consensus neighbour-joining tree:
...
Due to the random nature of the bootstrap sampling process, the results of these two programs are liable to change every time they are executed.
